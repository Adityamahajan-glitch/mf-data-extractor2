# === Streamlit AMFI NAV Dashboard with Progress Bar ===
        ...
        for line in lines[1:]:
            ...
            if split[0] == "":
                ...
                try:
                    ...
                    pass
            j += 1
        progress_bar.progress((i + 1) / len(ranges))
    return pd.DataFrame(nav_list)
# === UI ===
st.title("\U0001F4CA AMFI Mutual Fund NAV Dashboard")
st.markdown("Created using **Streamlit** | Data Source: [AMFI India](https://portal.amfiindia.com/)")
with st.sidebar:
    start_date = st.date_input("Fetch From Date", datetime(2022, 1, 1))
    end_date = st.date_input("Fetch To Date", datetime(2023, 12, 31))
    if st.button("\U0001F4E5 Fetch Data"):
