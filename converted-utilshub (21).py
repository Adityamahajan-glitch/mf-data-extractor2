...
if "df_nav" in st.session_state:
    ...
    # Premier RSI (Smoothed RSI using EMA of RSI)
    filtered_df["RSI_Premier"] = filtered_df["RSI_14"].ewm(span=5, adjust=False).mean()
    ...
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.04,
                        subplot_titles=["NAV & SMAs", "RSI (14) + Premier RSI", "MACD"])
    # --- Row 1: NAV + SMAs ---
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df["NAV"], name="NAV", line=dict(color="cyan")), row=1, col=1)
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df[f"SMA_{sma1}"], name=f"SMA {sma1}", line=dict(dash="dot", color="orange")), row=1, col=1)
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df[f"SMA_{sma2}"], name=f"SMA {sma2}", line=dict(dash="dot", color="lime")), row=1, col=1)
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df[f"SMA_{sma3}"], name=f"SMA {sma3}", line=dict(dash="dot", color="magenta")), row=1, col=1)
    # --- Row 2: RSI + Premier RSI ---
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df["RSI_14"], name="RSI (14)", line=dict(color="violet")), row=2, col=1)
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df["RSI_Premier"], name="Premier RSI", line=dict(color="yellow")), row=2, col=1)
    fig.add_shape(type="line", x0=min_date, x1=max_date, y0=70, y1=70,
                  line=dict(dash="dot", color="red"), row=2, col=1)
    fig.add_shape(type="line", x0=min_date, x1=max_date, y0=30, y1=30,
                  line=dict(dash="dot", color="green"), row=2, col=1)
    # --- Row 3: MACD ---
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df["MACD"], name="MACD", line=dict(color="aqua")), row=3, col=1)
    fig.add_trace(go.Scatter(x=filtered_df["Date"], y=filtered_df["Signal"], name="MACD Signal", line=dict(dash="dot", color="white")), row=3, col=1)
    # Layout
    fig.update_layout(
        height=900,
        title=f"{selected_scheme} - NAV Technical Chart",
        template="plotly_dark",
        hovermode="x unified"  # 👈 Crosshair hover syncing
    )
