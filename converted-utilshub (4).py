# === Streamlit AMFI NAV Dashboard with Login, Storage & Optimized Fetch ===
...
df_cache = pd.read_csv(CACHE_FILE, parse_dates=["Date"]) if os.path.exists(CACHE_FILE) else pd.DataFrame()
# === UI ===
st.title("📊 AMFI Mutual Fund NAV Dashboard")
st.markdown("Created using **Streamlit** | Source: [AMFI India](https://www.amfiindia.com/net-asset-value/nav-history)")
with st.sidebar:
    start_date = st.date_input("Fetch From Date", datetime(2023, 1, 1))
    end_date = st.date_input("Fetch To Date", datetime(2023, 12, 31))
    if st.button("📥 Fetch Data"):
        with st.spinner("Fetching data from AMFI..."):
            new_data = fetch_amfi_data(start_date.strftime('%d-%b-%Y'), end_date.strftime('%d-%b-%Y'))
            if new_data.empty:
                st.error("❌ No data returned from AMFI.")
            else:
                df_cache = pd.concat([df_cache, new_data]).drop_duplicates()
                df_cache.to_csv(CACHE_FILE, index=False)
                st.success(f"✅ Added {len(new_data)} new records.")
if df_cache.empty:
    st.warning("⚠️ No data loaded. Please fetch data using sidebar.")
    st.stop()
# === Filter & Visualize ===
selected_amc = st.selectbox("Select AMC", sorted(df_cache["AMC"].dropna().unique()))
schemes = df_cache[df_cache["AMC"] == selected_amc]["Name"].unique()
selected_scheme = st.selectbox("Select Scheme", schemes)
filtered_df = df_cache[df_cache["Name"] == selected_scheme].copy()
...
fig.add_shape(type="line", x0=f_date, x1=t_date, y0=70, y1=70, line=dict(dash="dot", color="red"), row=2, col=1)
fig.add_shape(type="line", x0=f_date, x1=t_date, y0=30, y1=30, line=dict(dash="dot", color="green"), row=2, col=1)
